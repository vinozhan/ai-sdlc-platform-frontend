...
import type { TestingSnapshot } from "../api/createTestingApi";
import { suiteTotals } from "../fixtures/runs";
import type {
  AppliedFix,
  AuditEntry,
  ConsoleLine,
...
function decorateFailures(input: ViewInput, confirmedThrough: number): ViewFailure[] {
  const baseFailures = input.data.failures;
...
function moduleCounts(failures: ViewFailure[], preview: PreviewState, modules: SuiteModule[]): SuiteModule[] {
  if (preview === "green") {
    return modules.map((m) => ({ ...m, passed: m.passed + m.failed, failed: 0, skipped: 0 }));
  }
...
function framesFromRecorded(
  lines: ConsoleLine[],
  progress: Record<number, { passed: number; failed: number }>,
): RunFrame[] {
  let current = { passed: 0, failed: 0 };
  return lines.map((line, i) => {
    if (progress[i]) current = progress[i];
 ...
 * would disagree.
 */
function framesForRerun(
  modules: SuiteModule[],
  failures: ViewFailure[],
  build: number,
  run: TestingSnapshot["run"],
): RunFrame[] {
  ...
  push({ kind: "muted", text: `re-running ${totals.run} tests · Build ${build} · ${run.branch}` });

  (["frontend", "backend"] as const).forEach((side) => {
    push({
      kind: "info",
      text: side === "frontend" ? `frontend · ${run.frontendRunner}` : `backend · ${run.backendRunner}`,
    });
  ...
  push({
    kind: "summary",
    text: `${totals.run} tests run · ${totals.passed} passed · ${totals.failed} failed · ${totals.skipped} skipped · ${run.duration}`,
  });
  push({
    kind: "muted",
    text:
...
function decorateFindings(input: ViewInput): Finding[] {
  const { findings: baseFindings, greenRun } = input.data;
  return baseFindings.map((finding) => {
    if (input.preview === "green") {
      const late = greenRun.lateFixes[finding.id];
      return {
        ...finding,
...
function decorateFixes(input: ViewInput, findings: Finding[]): AppliedFix[] {
  const { findings: baseFindings, appliedFixes: baseFixes } = input.data;
  if (input.preview === "green") {
...
function greenQuality(data: TestingSnapshot): Quality {
  const { quality: baseQuality, greenRun } = data;
  return {
    ...baseQuality,
...
export function buildView(input: ViewInput): TestingView {
  ...
  const modules = moduleCounts(failures, input.preview, suiteModules);
  const settled = suiteTotals(modules);

  const frames = green
    ? framesFromRecorded(greenRun.transcript, streamProgress)
    : isRerun
    ? framesForRerun(modules, failures, build, testingRun)
    : framesFromRecorded(input.preview === "running" ? streamingTranscript : runTranscript, streamProgress);
  ...
  const quality = green ? greenQuality(data) : baseQuality;
